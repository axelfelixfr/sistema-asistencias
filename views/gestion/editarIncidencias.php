<!-- Inicio de Editar Incidencias -->
<div class="container px-4 py-5">
    <?php if (isset($edit) && isset($inc) && is_object($inc)) : ?>
        <?php $url_action = base_url . "incidencia/editarIncidencia&clave=" . $inc->CCVEEMP . "&fecha=" . $inc->DFECINC . "&tipo=" . $inc->CTIPINC; ?>
    <?php endif; ?>
    <h1 class="pb-2 text-center text-danger border-bottom">Editar Incidencia</h1>
    <div class="container-fluid">
        <p class="mx-auto fw-bold text-warning"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle" viewBox="0 0 16 16">
            ...
            </svg> Se recomienda solo cambiar el status de la incidencia para conservar la integridad de los registros</p>
        <form action="<?= $url_action ?>" method="POST" enctype="multipart/form-data" class="pt-3 row gy-2 gx-3 align-items-center">
            <div class="pb-4 col-4">
                ...
                <input type="text" readonly class="form-control" id="clave" value="<?= isset($inc) && is_object($inc) ? $inc->CCVEEMP : ''; ?>" name="clave" required="true">
            </div>
            <div class="pb-4 col-2">
                <label class="form-label" for="fecha">Fecha de la incidencia</label>
                <input type="date" readonly name="fecha" id="fecha" value="<?= isset($inc) && is_object($inc) ? $inc->DFECINC : ''; ?>" pattern="^(0?[1-9]|1[0-2])[\/](0?[1-9]|[12]\d|3[01])[\/](19|20)\d{2}$" class="form-control" required="true" />
            </div>
            <div class="pb-4 col-3">
                <label for="tipo" class="form-label">Tipo de incidencia</label>
                <input type="text" readonly class="form-control" maxlength="1" id="tipo" name="tipo" required="true" value="<?= isset($inc) && is_object($inc) ? $inc->CTIPINC : ''; ?>">
            </div>
            <div class="col-3">
                <label for="status" class="form-label">Estatus</label>
                <input type="text" class="form-control" pattern="[A,I]{1}" aria-describedby="statusHelp" maxlength="1" id="status" value="<?= isset($inc) && is_object($inc) ? $inc->CSTATUS : ''; ?>" name="status" required="true">
            ...
            <div class="py-3 row d-flex align-content-center">
                <div class="text-center">
                    ...
                    <a class="btn btn-danger" href="<?= base_url ?>incidencia/gestion"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" class="bi bi-reply-all" viewBox="0 0 16 16">
